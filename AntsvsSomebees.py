...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        beePlace = self.place
        rangechecker=0
        while(beePlace != beehive):
            if(random_or_none(beePlace.bees) != None and rangechecker<=self.max_range and rangechecker>=self.min_range):
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.imposter==True:
            ...
            currentlocation = self.place
            ScubaThrower.action(self, gamestate)
            while currentlocation.exit:
                ant = currentlocation.exit.ant
                if isinstance(ant, ContainerAnt) and ant.contained_ant and (ant.contained_ant in self.doubleattack)==False:
                    ant.contained_ant.damage = 2 * ant.contained_ant.damage
                    self.doubleattack.append(ant.contained_ant)
                elif ant and (ant in self.doubleattack)==False:
                    ant.damage = 2 * ant.damage
                    self.doubleattack.append(ant)
...
def make_slow(action, bee):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    def another_action(gamestate):
        print("DEBUG:")
        if gamestate.time %2 ==0 :
            print("DEBUG: even turnß")
            action(gamestate)
